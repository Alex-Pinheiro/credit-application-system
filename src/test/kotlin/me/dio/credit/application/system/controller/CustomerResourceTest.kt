...
class CustomerResourceTest {
    ...
    fun `should create a customer and return 201 status` (){
        ...
        mockMvc.perform(MockMvcRequestBuilders.post(URL).contentType(MediaType.APPLICATION_JSON).content(valueAsString))
            ...
            .andExpect(MockMvcResultMatchers.jsonPath("$.cpf").value("68950276291"))
            .andExpect(MockMvcResultMatchers.jsonPath("$.email").value("babas@doctor.com"))
            .andExpect(MockMvcResultMatchers.jsonPath("$.zipCode").value("123456"))
            .andExpect(MockMvcResultMatchers.jsonPath("$.street").value("Minha rua"))
            .andExpect(MockMvcResultMatchers.jsonPath("$.id").value(1))
            .andDo(MockMvcResultHandlers.print())
    }

    @Test
    fun `should not save a customer with same CPF and return a 409 status`(){
    ...
    @Test
    fun `should not save a customer with first name empty and return 400 status`(){
